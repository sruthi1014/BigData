...
import models.resnetmymodel
import models.resnet_softmax
import models.resnet_twolayernn
import models.resnet
import models.onlyresnet
...
parser.add_argument('--model', choices=['softmax', 'convnet', 'twolayernn', 'mymodel', 'resnet_mymodel','resnet_softmax','resnet_twolayernn','resnet','onlyresnet'], help='which model to train/evaluate')
...
elif args.model == 'resnet_softmax':
    model = models.resnet_softmax.ResnetSoftmax(im_size, n_classes)
elif args.model == 'resnet_twolayernn':
    model = models.resnet_twolayernn.Resnettwolayer(im_size, args.hidden_dim, n_classes)
#RESNET
elif args.model == 'resnet':
    # CNN
    model = models.resnet.RESNET(im_size, args.hidden_dim, args.kernel_size, n_classes)
#ONLY RESNET
elif args.model == 'onlyresnet':
    # Question 6d
    model = models.onlyresnet.ONLYRESNET(im_size, args.hidden_dim, args.kernel_size, n_classes)
elif args.model == 'resnet_mymodel':
    # Problem 5 (multiple hidden layers, input -> hidden layers -> output)
    print("sruthi check 1")
    model = models.resnetmymodel.ResnetMyModel(im_size, args.hidden_dim, args.kernel_size, n_classes)
else:
